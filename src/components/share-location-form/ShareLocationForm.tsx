...
import { addNewSharedLocation } from '../../store/slices/SharedLocationSlice';
...
import { editSelectedLocation } from '../../store/slices/retrieveSelectedLocationDataSlice';
...
const ShareLocationForm: React.FC = () => {
  ...
  const initialLocationName = useAppSelector(
    state => state.selectedLocation.locationName
  );
  const [locationNameInputValue, setLocationNameInputValue] =
    useState(initialLocationName);

  const initialLocationType = useAppSelector(
    state => state.selectedLocation.locationType
  );

  const selectedLocationId = useAppSelector(
    state => state.selectedLocation.locationId
  );
  const [locationTypeInputValue, setLocationTypeInputValue] =
    useState(initialLocationType);
  ...
  const locationIsSavedBefore = useAppSelector(
  ...
  const locationSaved = useAppSelector(
    state => state.selectedLocation.locationSaved
  );
  const cancelButtonClickHandler = (event: React.MouseEvent<HTMLElement>) => {
    locationSaved && dispatch(hidePopupContent());
    dispatch(toggleModal());
    dispatch(cancelSavingSharedLocation());
  };
  ...
  const shareLocationFormHandler = (e: FormEvent) => {
    ...
    locationIsSavedBefore &&
      dispatch(
        editSelectedLocation({
          locationName: locationNameInputValue,
          locationType: locationTypeInputValue,
          locationLat: ShareLocationPosition.latitude,
          locationLng: ShareLocationPosition.longitude,
